...
description: 
published: false
date: 2025-08-25T20:55:58.252Z
tags: 
editor: markdown
dateCreated: 2025-06-17T18:33:25.191Z
---

> Вариант использования (ВИ) — все способы достижения определенного результата конкретным пользователем при взаимодействиии с ФС, где способы достижения результата описываются в виде сценария использования.
{.is-info}
...
1. Ключевым моментом ВИ является именно достигаемая цель, которая выражает пользу от использования ФС. 

2. Для случая цФС ВИ формулируется существительным, обозначающий артефакт данных определенной (АРД) структуры, и глаголом действия, обозначающий действие над АРД.

```json
{глагол-действия} {артефакт-данных}
```

> Гипотеза: любую функцию ФС можно записать в формате ВИ
{.is-info}


3. Сценария использования ВИ включает взаимодействие цИС и Пользователя, т.о. пользователь является неотъемлемым элементом исполнения функции цИС.

4. Функцию ВИ можно сделать чистой, если контекст исполнения сделать входящими параметрами функции.
